...
function buildDestinationURL(incrementVersion) {
  const version = incrementVersion ? CTO_DOCUMENTARY_VERSION + 1 : CTO_DOCUMENTARY_VERSION;
...
/**
 * Given a JobTemplate-like object, create it in AWS and make it official.
 * @param {JobTemplate} template The template object to create in AWS
 * @returns {JobTemplate}
 */
...
export async function getDocumentaryTemplate() {
  return getMostRecentJobTemplate({ Category: 'documentary' });
}
