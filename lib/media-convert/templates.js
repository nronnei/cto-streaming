 ...
 * @returns {JobTemplate}
 */
export async function upgradeTemplate(template) {
  try {
    ...
    _handleError('upgradeLastTemplate', error)
  }
}

export async function createJobTemplate(template) {
  try {
    const createTemplateParams = new CreateJobTemplateCommand(template);
    const res = await client.send(createTemplateParams);
